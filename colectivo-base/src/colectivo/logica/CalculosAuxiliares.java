...
/**
 * Encapsula los métodos de cálculo auxiliares compartidos por todas las
...
public class CalculosAuxiliares {
	...
	private static LocalTime obtenerProximaHoraSalida(Linea linea, int diaSemana, LocalTime horaLlegadaUsuario,
		...
		try {
			...
			// 1. Obtener los horarios de HOY
			List<LocalTime> horasHoy = linea.getHorasFrecuencia(diaSemana);
			if (horasHoy == null || horasHoy.isEmpty()) {
				logger.debug("No hay horas de frecuencia para línea {} en día {}", linea.getNombre(), diaSemana); // ✅
																													// DEBUG
																													// en
																													// lugar
																													// de
			...
			// 2. Calcular la hora de salida mínima requerida de la cabecera
			...
			// 4. Si encontramos un bus para HOY, ¡perfecto! Ese es el mejor.
			if (mejorHoraSalidaHoy != null) {
				LocalTime horaLlegada = mejorHoraSalidaHoy.plusSeconds(duracionHastaParada);
				logger.debug("Salida encontrada hoy: {} (llegada a parada: {})", mejorHoraSalidaHoy, horaLlegada); // ✅
																													// LOG
																													// DE
																													// ÉXITO
			...
			// 5. Si NO encontramos bus en "hoy" (porque ya pasaron todos, ej: son las
			// 23:55),
			// entonces buscamos el primer bus de MAÑANA.

			// 5a. Calcular qué día es "mañana" (manejando el cambio de 7 a 1)
			int diaSiguiente = (diaSemana % 7) + 1;

			// 5b. Obtener los horarios de MAÑANA
			List<LocalTime> horasManana = linea.getHorasFrecuencia(diaSiguiente);

			LocalTime mejorHoraSalidaManana = null;

			// 5c. Búsqueda en MAÑANA:
			// Encuentra el bus MÁS TEMPRANO de mañana (el primero del día).
			if (horasManana != null) {
			...
			// 6. Si encontramos un bus para mañana, lo devolvemos.
			if (mejorHoraSalidaManana != null) {
				LocalTime horaLlegada = mejorHoraSalidaManana.plusSeconds(duracionHastaParada);
				logger.debug("Salida encontrada mañana (día {}): {} (llegada a parada: {})", diaSiguiente,
						mejorHoraSalidaManana, horaLlegada);
				// Devuelve la hora de llegada a la parada (ej: 06:10 de mañana)
				return horaLlegada;
			}

			// 7. Si no hay buses ni hoy ni mañana (lista vacía), no hay ruta.
			logger.warn("No hay salidas disponibles para línea {} en día {} ni día siguiente", linea.getNombre(),
					diaSemana);
			return null;

		} catch (Exception e) {
			logger.error("Error inesperado al obtener la próxima hora de salida para línea {}", linea.getNombre(), e);
			return null;
		}
	}
